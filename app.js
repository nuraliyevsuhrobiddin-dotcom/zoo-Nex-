...
const GLOBAL_AD_CONTACT = {
    phone: "+998997777031",
    telegram: "Nuraliyev1s"
};
...
try {
    const savedAcademyCompleted = localStorage.getItem('zootube_academy_completed');
    state.academyCompleted = savedAcademyCompleted ? JSON.parse(savedAcademyCompleted) : {};
} catch (e) {
    state.academyCompleted = {};
}
...
function applyGlobalAdContactToExistingVideos() {
    var touched = false;
    (state.videos || []).forEach(function (v) {
        if (!v) return;
        if (v.phone !== GLOBAL_AD_CONTACT.phone || v.telegram !== GLOBAL_AD_CONTACT.telegram) {
            v.phone = GLOBAL_AD_CONTACT.phone;
            v.telegram = GLOBAL_AD_CONTACT.telegram;
            touched = true;
        }
    });
...
function renderVideos() {
    ...
    list.forEach(v => {
        ...
        const cardPhone = (v.phone || GLOBAL_AD_CONTACT.phone).replace(/\D/g, '');
        const cardTg = (v.telegram || GLOBAL_AD_CONTACT.telegram).replace('@', '').trim();
            ...
            <div class="video-info">
                ...
                <div class="details">
                    ...
                    <div style="display:flex; gap:8px; margin-top:8px;">
                        <a href="tel:${cardPhone}" onclick="event.stopPropagation()" style="font-size:12px; padding:6px 10px; border-radius:8px; text-decoration:none; background:#25D366; color:#111; font-weight:600;">
                            <i class="fas fa-phone"></i> Qo'ng'iroq
                        </a>
                        <a href="https://t.me/${cardTg}" target="_blank" onclick="event.stopPropagation()" style="font-size:12px; padding:6px 10px; border-radius:8px; text-decoration:none; background:#2AABEE; color:#111; font-weight:600;">
...
function openVideoPlayer(video) {
    ...
    // Contacts
    const phone = (video.phone || GLOBAL_AD_CONTACT.phone).replace(/\D/g, '');
    const tgUsername = (video.telegram || GLOBAL_AD_CONTACT.telegram).replace('@', '').trim();
    const tgBtn = document.getElementById('tgContactBtn');
    const waBtn = document.getElementById('waContactBtn');
    if (tgBtn) tgBtn.href = tgUsername ? `https://t.me/${tgUsername}` : `https://wa.me/${phone}`;
...
function saveAcademyCompleted() {
    localStorage.setItem('zootube_academy_completed', JSON.stringify(state.academyCompleted || {}));
}

function getCurrentUserCompletedCourses() {
    if (!state.currentUser || !state.currentUser.email) return [];
    var key = state.currentUser.email;
    if (!state.academyCompleted[key]) state.academyCompleted[key] = [];
    return state.academyCompleted[key];
}

function isAcademyCourseCompleted(courseId) {
    return getCurrentUserCompletedCourses().includes(courseId);
}

function completeAcademyCourse(courseId) {
    if (!state.currentUser) {
        openAuthModal('login');
        return showToast("Kursni tugatganingizni belgilash uchun tizimga kiring", "info");
    }
    var completed = getCurrentUserCompletedCourses();
    if (!completed.includes(courseId)) {
        completed.push(courseId);
        state.academyCompleted[state.currentUser.email] = completed;
        saveAcademyCompleted();
        showToast("Tabriklaymiz! Kurs yakunlandi. Endi sertifikat olishingiz mumkin.");
    } else {
        showToast("Bu kurs allaqachon yakunlangan.", "info");
    }
    renderAcademy();
}
window.completeAcademyCourse = completeAcademyCourse;

function downloadAcademyCertificate(courseId) {
    if (!state.currentUser) {
        openAuthModal('login');
        return showToast("Sertifikat olish uchun tizimga kiring", "info");
    }
    if (!isAcademyCourseCompleted(courseId)) {
        return showToast("Avval kursni tugatganingizni belgilang", "error");
    }

    var course = (state.academyCourses || []).find(function (c) { return c.id === courseId; });
    if (!course) return showToast("Kurs topilmadi", "error");

    var learner = state.currentUser.name || "O'quvchi";
    var now = new Date();
    var issueDate = now.toLocaleDateString('uz-UZ');
    var certNo = "ZNX-" + now.getFullYear() + "-" + String(courseId).replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
    var certificateHtml = `
        <!doctype html>
        <html lang="uz">
        <head><meta charset="UTF-8"><title>Sertifikat</title></head>
        <body style="font-family:Arial,sans-serif; padding:40px; background:#f7f7f7;">
            <div style="max-width:800px; margin:auto; background:white; border:3px solid #d9a441; border-radius:12px; padding:36px;">
                <h1 style="margin:0; text-align:center; color:#b37b00;">ZooNex Akademiya Sertifikati</h1>
                <p style="text-align:center; color:#666; margin:8px 0 24px;">Sertifikat raqami: <strong>${certNo}</strong></p>
                <p style="font-size:18px; text-align:center;">Ushbu sertifikat</p>
                <h2 style="text-align:center; margin:8px 0 18px;">${learner}</h2>
                <p style="text-align:center; font-size:18px;">ning <strong>${course.title}</strong> kursini muvaffaqiyatli yakunlaganini tasdiqlaydi.</p>
                <p style="text-align:center; margin-top:30px;">Berilgan sana: <strong>${issueDate}</strong></p>
                <div style="display:flex; justify-content:space-between; margin-top:60px;">
                    <div>
                        <div style="border-top:1px solid #222; width:220px;"></div>
                        <small>Akademiya koordinatori</small>
                    </div>
                    <div>
                        <div style="border-top:1px solid #222; width:220px;"></div>
                        <small>Platforma muhri: ZooNex</small>
                    </div>
                </div>
            </div>
        </body>
    ...
    var certWindow = window.open('', '_blank');
    if (!certWindow) return showToast("Brauzer popup-ni blokladi. Iltimos ruxsat bering.", "error");
    certWindow.document.write(certificateHtml);
    certWindow.document.close();
    showToast("Sertifikat tayyorlandi. Yangi oynadan saqlab oling.");
}
window.downloadAcademyCertificate = downloadAcademyCertificate;
...
function renderAcademy() {
    ...
    aGrid.innerHTML = `
        <div class="dispatch-post-cta" style="grid-column:1/-1; margin-bottom: 20px;">
            <div>
                ...
                <p style="color:var(--text-secondary);margin:0;font-size:13px;">Kurs yakunlangach har bir o'quvchiga sertifikat beriladi</p>
            </div>
    ...
    state.academyCourses.forEach(c => {
        ...
        const isCompleted = isAcademyCourseCompleted(c.id);
        const card = document.createElement('div');
        ...
        card.innerHTML = `
            ...
            <div class="course-info">
                ...
                <div style="display:flex; gap:8px; margin-top:10px; flex-wrap:wrap;">
                    <button class="course-btn" onclick="completeAcademyCourse('${c.id}')" style="background:${isCompleted ? '#2e7d32' : '#4CAF50'}; color:white;">
                        <i class="fas fa-check-circle"></i> ${isCompleted ? "Tugatildi" : "Kursni tugatdim"}
                    </button>
                    <button class="course-btn" onclick="downloadAcademyCertificate('${c.id}')" style="background:${isCompleted ? '#ffd54f' : '#777'}; color:#111;">
                        <i class="fas fa-certificate"></i> Sertifikat olish
                    </button>
                </div>
                ${isCompleted ? '<div style="margin-top:8px; font-size:12px; color:#8bc34a;"><i class="fas fa-certificate"></i> Sertifikatga ega</div>' : ''}
            </div>
        `;
...
function init() {
    ...
    applyGlobalAdContactToExistingVideos();
